...
$total = collect($detalle)->sum('subtotal');

/* ===== CONFIGURACIÓN DE PAGINACIÓN ===== */
$filasPorPagina = 21;
$paginas = collect($detalle)->chunk($filasPorPagina);

/* ===== FUNCIONES NÚMERO A LETRAS ===== */
function numeroALetras($numero) {
    ...
    if ($entero == 0) {
        ...
        if ($entero > 0) {
            $texto .= convertirCientos($entero, $unidades, $decenas, $especiales, $centenas);
        }
    }

    $texto = trim($texto);
    $resultado = $texto . ' QUETZALES';
    $resultado .= ($decimales != '00')
        ? ' CON ' . numeroALetrasCorto($decimales) . ' CENTAVOS'
        : ' EXACTOS';
        ...
        if ($numero >= 10 && $numero <= 19) {
            ...
            if ($decena > 0) {
                if ($decena == 2 && $unidad > 0) {
                    $texto .= 'VEINTI' . strtolower($unidades[$unidad]) . ' ';
                } else {
                    $texto .= $decenas[$decena] . ($unidad > 0 ? ' Y ' : ' ');
                }
            }
            if ($unidad > 0 && !($decena == 2 && $unidad > 0)) {
...
<!-- ================= DATOS SUPERIORES ================= -->
<div class="row">
    <div class="col w-6 box"><div class="label">DIA</div>{{ $fecha->format('d') }}</div>
    <div class="col w-6 box"><div class="label">MES</div>{{ $fecha->format('m') }}</div>
    <div class="col w-8 box"><div class="label">AÑO</div>{{ $fecha->format('Y') }}</div>
    <div class="col w-50 box left"><div class="label">Proveedor.:</div>{{ $header->proveedor_nombre }}</div>
    <div class="col w-25 box"><div class="label">NIT.:</div>{{ $header->proveedor_nit }}</div>
</div>

<div class="row">
    <div class="col w-70 box left"><div class="label">Dirección.:</div>{{ $header->proveedor_direccion }}</div>
    <div class="col w-30 box"><div class="label">Tel.:</div></div>
</div>

<div class="row">
    <div class="col w-20 box"><div class="label">Número Factura.:</div>{{ $header->numero_factura }}</div>
    <div class="col w-80 box"><div class="label">Serie FAC.:</div>{{ $header->serie_factura }}</div>
</div>

<div class="separator"></div>

<p>Señor Guardalmacén, procedente del proveedor identificado en casilla anterior, sírvase recibir lo siguiente:</p>
...
<tr>
    ...
    <td width="24%" class="right total-row">{{ number_format($total,2) }}</td>
</tr>
</table>
...
<div class="row">
    <div class="col w-40 firma-container">
        <span class="texto-firma">_______________________________<br>FIRMA ENCARGADO</span>
    </div>
    <div class="col w-20"></div>
    <div class="col w-40 firma-container">
        <span class="texto-firma">{{ $header->proveedor_nombre }}<br>FIRMA DEL PROVEEDOR</span>
    </div>
</div>
