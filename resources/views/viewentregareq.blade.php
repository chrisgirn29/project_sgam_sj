...
function guardarRequisicion() {
    ...
    const descripcion = document.querySelector('[name="detalle"]').value.trim();
    ...
    if (descripcion.length < 20) {
        Swal.fire({
            ...
            text: 'Es necesario que justifique su Solicitud',
    ...
    .then(resp => {
        ...
        if (!resp.ok) {
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: resp.error ?? 'No se pudo guardar'
            });
            btn.disabled = false;
            return;
        }
...
function limpiarFormulario() {
    ...
    document.querySelector('[name="tipo_solicitud"]').value = '';
    document.querySelector('[name="detalle"]').value = '';

    // ===== EMPLEADO =====
    document.getElementById('inputEmpleado').value = '';
    document.getElementById('selectEmpleado').value = '';
    document.getElementById('unidadEmpleado').textContent = '—';
    document.getElementById('puestoEmpleado').textContent = '—';

    // ===== PROGRAMA =====
    document.getElementById('inputPrograma').value = '';
    document.getElementById('selectPrograma').value = '';
    ...
    document.getElementById('totalGeneral').textContent = 'Q 0.00';
