...
<form id="formRecepcion" method="POST" action="{{ route('entregas.store') }}">
                ...
                <div class="bg-white border-2 border-primary rounded-xl p-4">
                    ...
                    <input type="hidden"
                        ...
                        <button type="button"
                                ...
                                class="mt-4 bg-[#F59C27] text-white w-full py-2 rounded font-semibold
                                    flex items-center justify-center gap-2
                                    hover:bg-[#e0ad23] transition">
                ...
                <div class="bg-white border-2 border-primary rounded-xl p-4">
                    ...
                    <table class="w-full border mt-2">
                        ...
                        @foreach($requisicion->detalles as $d)
                            @php
                                $cantidadEntregada = $d->cantidad_entregada ?? 0;
                                $cantidadPendiente = $d->cantidad - $cantidadEntregada;
                            @endphp
...
<script>
    ...
    Swal.fire({
        title: 'Documento de Entrega o Despacho.:',
        html: `
            <input id="documento" class="swal2-input" placeholder="Número de documento">
        `,
        confirmButtonColor: colorPrincipal,
        showCancelButton: true,
        preConfirm: () => {
            if (!documento.value) {
                Swal.showValidationMessage('Ingrese el número de documento.:');
                return false;
            }
        ...
        if (r.isConfirmed) {
            // SOLO ESTE CAMPO
            numero_documento.value = r.value;

            // LOS DEMÁS SE ENVÍAN VACÍOS
            serie_factura.value = null;
